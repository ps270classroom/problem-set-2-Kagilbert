...
knitr::kable(incumbency_table,
             col.names = c("Incumbency Staus", "Number of Candidates"),
             digits = 0,
             caption = "Number of Candidates by Incumbency Status")
```
...
party_incumbent_promote <- filtered_data |>
  ...
  arrange(party, incumbency)

knitr::kable(party_incumbent_promote,
             digits = 3, 
             col.names = c("Party", "Incumbency Status", "Promote Proportion"),
             caption = "Average Promote Proportion by Party and Incumbency")
```
The candidate with the highest average is an Incumbent Democrat with an average promote proportion of 0.854. 
...
plot_q3 <- ggplot(us_house_data, aes(x = impressions_millions)) + 
  geom_histogram() + 
  labs(title = "Histogram of Facebook Ad Impressions (millions) for US House Races",
       x = "Impressions (millions)",
       y = "Count")

plot_q3
```
The histogram is skewed to the right, meaning that the majority of Candidates had fewer than 10 million impressions. 
